 ...
 #   """ Use the model from https://github.com/abhiskk/fast-neural-style/blob/master/neural_style/utils.py """
  #  if not os.path.exists(model_dir):
   #     os.mkdir(model_dir)
    ...
    #return vgg
def load_vgg16(model_dir):
    """ Load VGG16 model and save its weights if not already saved. """
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)

    weight_path = os.path.join(model_dir, 'vgg16_features.pth')
    if not os.path.exists(weight_path):
        # 加载 torchvision 提供的预训练模型，只保留特征提取部分
        vgg = models.vgg16(pretrained=True)  # 使用 pretrained=True 来加载预训练模型
        vgg_features = vgg.features  # 只保留卷积层部分

        # 保存模型的特征提取部分
        torch.save(vgg_features.state_dict(), weight_path)
    else:
        # 加载保存的卷积特征提取部分
        vgg_features = models.vgg16().features  # 只初始化卷积部分
        vgg_features.load_state_dict(torch.load(weight_path))
...
def vgg_preprocess(batch):
    ...
    if batch.shape[1] == 1:
        # 直接将 [-1, 1] 转换到 [0, 255] 范围
        batch = (batch + 1) * 255 * 0.5
        # 对单通道图片进行均值减法，可以设置一个通用的均值，如 117.0
        mean = 117.0  # 通常灰度图像的平均值
        batch = batch - mean
    else:
